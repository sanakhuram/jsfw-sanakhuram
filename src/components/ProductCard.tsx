...
export default function ProductCard({
    ...
    return (
        <div className="bg-gray-200 rounded-xl shadow p-4 relative">
            ...
            <Link href={`/product/${id}`} className="relative block w-full h-48 rounded-md mb-4 overflow-hidden">
                <Image
                    src={image.url}
                    alt={image.alt}
                    fill
            ...
            <p className="text-yellow-600 mb-2 text-sm">{renderStars(rating)} ({rating})</p>

            <p className="mb-2">
                {discountedPrice < price ? (
                    <>
                        <span className="line-through text-gray-800 mr-2">
                            ${price.toFixed(2)}
                        </span>
